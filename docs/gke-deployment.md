...
# Expected output: YOUR_PROJECT_ID.svc.id.goog
```

**Configure Workload Identity for Vault:**
```bash
# Create GCP service account
gcloud iam service-accounts create vault-gke-sa \
  --display-name="Vault GKE Service Account"

# Grant necessary permissions (example: access to Cloud KMS for auto-unseal)
gcloud projects add-iam-policy-binding YOUR_PROJECT_ID \
  --member="serviceAccount:vault-gke-sa@YOUR_PROJECT_ID.iam.gserviceaccount.com" \
  --role="roles/cloudkms.cryptoKeyEncrypterDecrypter"

# Bind Kubernetes service account to GCP service account
gcloud iam service-accounts add-iam-policy-binding \
  vault-gke-sa@YOUR_PROJECT_ID.iam.gserviceaccount.com \
  --role roles/iam.workloadIdentityUser \
  --member "serviceAccount:YOUR_PROJECT_ID.svc.id.goog[vault/vault]"

# Annotate Kubernetes service account
kubectl annotate serviceaccount vault \
  -n vault \
  iam.gke.io/gcp-service-account=vault-gke-sa@YOUR_PROJECT_ID.iam.gserviceaccount.com
```
...
# standard-rwo (default): Standard persistent disk with CSI driver
# premium-rwo: SSD persistent disk with CSI driver
# standard (legacy): Standard persistent disk (deprecated)
```

**Vault Uses Cluster Default:**
The Vault Helm chart uses the cluster's default storage class (`standard-rwo`):
```yaml
server:
  dataStorage:
    enabled: true
    size: 10Gi
    # storageClass not explicitly set, uses cluster default
```

**Change to Premium SSD (Optional):**
```bash
# Edit Vault values during installation
helm upgrade vault hashicorp/vault \
  --namespace vault \
  --set server.dataStorage.storageClass=premium-rwo
```
...
**Internal Load Balancer (Private Cluster):**
```yaml
service:
  type: LoadBalancer
  annotations:
...
```bash
# Get external IP
export VAULT_ADDR=$(kubectl get svc -n vault vault -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
...
**Enable Network Policy (Already enabled):**
```bash
# Verify network policy is enabled
gcloud container clusters describe vault-gke-cluster --region us-central1 --format="value(networkPolicy.enabled)"

# Expected output: true
```

**Example Network Policy for Vault:**
```yaml
...
- Standard cluster: Free
- Autopilot cluster: Pay per pod resource usage

**Compute Instances (3 x e2-medium):**
- On-Demand: ~$75/month
- Preemptible: ~$22/month (70% savings)
- Spot VMs: ~$22/month (similar to preemptible)

**Persistent Disks:**
- 3 x 50 GB (node storage): ~$6/month (standard)
- 1 x 10 GB (Vault PVC): ~$0.40/month (standard)
- Premium SSD: 2.5x more expensive

**Networking:**
- Cloud NAT: ~$45/month (includes data processing)
- External IP: ~$7/month
- Egress Traffic: ~$10-50/month (varies by usage)

**Load Balancer:**
- External TCP/UDP Load Balancer: ~$18/month

**Total Estimated Cost:**
- On-Demand: ~$160-210/month
- With Preemptible VMs: ~$110-160/month

**Cost Optimization Tips:**
- Use preemptible or spot VMs for non-critical workloads
- Use regional persistent disks for HA (slightly more expensive)
- Enable cluster autoscaler to scale down during off-hours
- Use committed use discounts for predictable workloads
- Use standard persistent disks instead of SSD for non-performance-critical workloads
- Consider Autopilot for simplified management and potential cost savings
...
gcloud compute networks list --filter="name:vault-gke-network"
```

### Cleanup Verification

```bash
# Verify no GKE clusters
gcloud container clusters list --region us-central1

# Verify no VPC networks
gcloud compute networks list --filter="name:vault-gke-network"

# Verify no load balancers
gcloud compute forwarding-rules list

# Verify no persistent disks
gcloud compute disks list --filter="name~vault"

# Verify no service accounts
gcloud iam service-accounts list --filter="email:vault-gke-sa@"
```
...
1. **High Availability:**
   - Use regional clusters (multi-zonal control plane)
   - Deploy Vault in HA mode (3+ replicas)
   - Use regional persistent disks for data replication
   - Distribute workloads across multiple zones

2. **Security:**
   - Enable Workload Identity for GCP service integration
   - Use private clusters with authorized networks
   - Implement network policies
   - Enable Binary Authorization for image verification
   - Use GKE Sandbox for enhanced pod isolation
...
4. **Backup:**
   ...
   - Store snapshots in Cloud Storage with versioning
   - Enable persistent disk snapshots
   - Test restore procedures regularly
   - Use regional persistent disks for automatic replication
...
5. **Cost Optimization:**
   ...
   - Use standard persistent disks for non-performance-critical workloads
...
- Smaller machine types (e2-small)
- Fewer nodes (1-2 total)
...
- Larger machine types (e2-standard or n2-standard)
- More nodes (6+ total)
- Premium SSD persistent disks
